...
* :data:`BOOK_DENSE_SCRIPT_PROMPT` — the script generator system prompt that
  keeps the final spoken lines dense but full-length (6-12 hard points,
  ~180-240s / ~1000-1400 汉字) and asks for chapter-relevant keywords.
...
BOOK_DENSE_REWRITE_PROMPT = (
    ...
    "任务：把输入章节压缩为 6-12 条硬核要点，只保留事实、机制、关键转折与数字。\n"
    "要求：\n"
    "1. 删除序言套话、目录腔、作者生平、重复铺陈，以及与主线无关的旁枝\n"
    "2. 用口语化、可直接朗读的中文短句，一条要点一到两句话\n"
    "3. 不要 markdown、不要小标题、不要“本章将……”之类的过渡句\n"
    "4. 全文约 1000-1400 字，朗读约 180-240 秒，信息密度要高，不要注水\n"
    "只输出压缩后的口播稿正文，不要任何解释。"
)
...
BOOK_DENSE_SCRIPT_PROMPT = (
    ...
    "规则：\n"
    "1. 只保留事实、机制、关键转折和数字，写成 6-12 个要点\n"
    "2. 删除序言套话、目录腔、作者生平、与主线无关的旁枝\n"
    "3. 口语化短句，第一句是钩子，不要空话；要点之间要连贯且不重复\n"
    "4. 全文朗读控制在 180-240 秒（约 1000-1400 字），总时长目标 210 秒\n"
    "5. 拆成 6-12 个段落，每段朗读约 20-35 秒，每段 2-3 个具体配图关键词\n"
