...
- Media is **deduplicated per episode** (Pexels id + filename), so the same
  video or image is never reused across segments. Each segment searches with one
  short, **chapter-anchored** query (1–2 anchor terms + up to 2 narrower terms),
  which keeps consecutive visuals on the same theme instead of jumping topics.
